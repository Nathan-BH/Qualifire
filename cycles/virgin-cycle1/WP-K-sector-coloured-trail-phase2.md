**Status: Brief written 2026-09-03 (Digest+Plan pass). Ready to execute — Nathan's Q7 answer is fully incorporated (live map + RIDES popup + settings toggle, all three in scope). No decision from Nathan needed before execution (§7 lists defaults; the one thing worth a glance is §7.2, the live gate ticks, which is deliberately NOT changed here). Core design: one new pure module `sectorTrailModel.ts` (two builders `liveSectorColours` / `storedSectorColours` + the `ALL_YELLOW` sentinel, tier→colour injected as `tierLineColour` per demoModel's precedent) feeds RouteMapView's already-generic `sectorColours` prop from all three surfaces, gated by one new `Settings.sectorColours` toggle (default ON); the sector-spans source moves above the WP-J trail in `routeMapView.tsx` because the trail's width-7 casing otherwise fully hides a width-6 span on the live map; no `leadColour` on the live map.**
