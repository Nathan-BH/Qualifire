**Status: Brief written 2026-09-03 (Digest+Plan pass). Ready to execute — Nathan's Q2 answer is incorporated via the coordinator's interpretation (free-text segments, per-Route, prefix-grouped in RECORD); no decision from Nathan needed to start (§9 lists two product defaults he may want to override later, neither blocks). Core design: `Route.specs?: string[]` (ordered free-text segments after the Way's From/To), `draftWayCreation` stops refusing an existing directed Way and instead returns a draft with `existingWayId` set so `buildWayCreationCatalog` mints a new Route on that Way, the naming card gains a spec-segment input with prefix-aware suggestion chips, and RECORD derives its pill rows from a per-depth grouping of the picked route's spec path (`specPickRows`) — falling back byte-for-byte to today's flat row whenever no route on the way has specs.**
