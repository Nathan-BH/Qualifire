**Status: DONE — landed on the device 2026-09-04, independently inspected (fresh Fable pass, clean; 5 non-blocking notes, none code bugs — see README/OPEN-ITEMS). 451 tests, 448 pass, 0 fail, 3 skip (13 new); `tsc --noEmit` exit 0. `gateAdjustCard.tsx`'s map area now draws the rider's real decimated ride line (via new `gateAdjustMapModel.ts`) with every gate placed by exact chainage; the pad is now 4 buttons (±1%/±0.1% of route length, `nudgeDeltaM`) replacing the old fixed ±10m/±50m pair, per Nathan's Q2 answer. Scrub gesture (§3.5) correctly left unbuilt, as Nathan ruled. Shared `GateAdjustDraft` (from WP-H's `wayFromRide.ts`) wired with the new `refLine` prop at both call sites (RecordScreen + RideDetailScreen) from one construction site.**
