...
## Status at a glance (2026-09-03 — A/B/C/D/F/J/L/N/O/P/Q landed; E/G/H/I/K/M now fully briefed and ready to execute — H and I each carry one flagged product question for Nathan, see their rows)
...
| E | Virgin manifest gate-leak (bundled gates drawn on new>>new free rides) | **Brief written 2026-09-03 (Digest+Plan pass) — ready to execute.** No decision needed from Nathan. The free-ride "black circles" leak turned out to already be closed by a prior WP-C change (`allRouteAssets` filters by the runtime catalog) — this brief makes that closure structural rather than incidental with a single build-mode guard (`bundledForSeedMode` in `seed.ts`) that empties the bundled manifest/PNGs everywhere `routeMapView.tsx` references them on a `SEED_MODE==='empty'` build; DEMO stops importing the shared manifest and gets its own small decoupled fixture. | `WP-E-virgin-manifest-leak.md` |
| G | Specifications / route variants on an existing Way | **Brief written 2026-09-03 (Digest+Plan pass) — ready to execute.** No decision needed from Nathan (two product-taste defaults noted in the brief's §9, neither blocking). Routes gain an optional ordered `specs: string[]` field; a repeat ride on a Way that already exists now offers to name a new variant of it instead of silently producing no offer; RECORD's route picker groups pills by shared spec prefix (`specPickRows`), falling back byte-for-byte to today's flat pill row whenever no route on a way has specs. | `WP-G-specifications-route-variants.md` |
| H | Ride detail screen (RIDES tap + post-stop destination) | **Brief written 2026-09-03 (Digest+Plan pass) — ready to execute, with one flagged product question.** Q4 (drop RESULTS entirely) is incorporated; the nav-model call is resolved — no navigation library is added, a lifted `App.tsx` overlay state mirrors the existing `recFullscreen` full-screen pattern. One genuine open question for Nathan, **not defaulted**: should "set as reference" also let a later ride replace an *existing* route's reference line (not just create a brand-new way)? See the brief's §8.1 — WP-H ships only the create-a-new-way seam either way. | `WP-H-ride-detail-screen.md` |
| I | Gate card on the map + finger scrub | **Brief written 2026-09-03 (Digest+Plan pass) — map half ready to execute; scrub half designed but explicitly NOT authorized.** Q1 clearly wants the gate-adjust card drawing gates on the real ride line instead of today's flat bar (fixed here, zero new dependencies, zero changes to `RouteMapView`/`routeAssetRuntime.ts`). It does **not** clearly authorize the new finger-scrub gesture on top of the kept ± pad — that's sent back to Nathan as a genuine open question in the brief's §7.1 rather than defaulted either way; do not execute §3.5 until he answers it. | `WP-I-gate-card-map-scrub.md` |
| K | Sector-coloured trail, phase 2 (live map) | **Brief written 2026-09-03 (Digest+Plan pass) — ready to execute.** No decision needed from Nathan. One shared pure module (`sectorTrailModel.ts`) feeds the already-generic `sectorColours` prop from all three surfaces Nathan named (live map, RIDES-tab row, Result), gated by one new settings toggle (default on); a real bug the digest surfaced along the way (WP-J's breadcrumb casing would have fully hidden the live sector line) is fixed as part of the same change. Deliberately leaves the separate gate-tick colouring untouched, per Nathan's own "gates should not change colour" line. | `WP-K-sector-coloured-trail-phase2.md` |
| M | RECORD setup layout (tight-and-grows vs fixed) | **Brief written 2026-09-03 (Digest+Plan pass) — ready to execute.** No decision needed from Nathan. Confirmed the live map (Q5's other half) is already on by default via WP-D (`settings.liveMap` defaults `true`); the only remaining work is a one-line style fix (the setup screen's `content` container drops its vertical centring so 2 pills stop reading as floating in empty space — the pills themselves were already flush-left and wrapping). | `WP-M-record-setup-layout.md` |
