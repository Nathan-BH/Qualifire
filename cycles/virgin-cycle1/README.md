...
## Status at a glance (2026-09-03 — all fully-briefed WPs landed: A/B/C/D/F/J/L/N/O/P/Q done; E/G/H/I/K/M need a Digest+Plan pass)
...
| E | Virgin manifest gate-leak (bundled gates drawn on new>>new free rides) | Not started — **Q6 answered** ("remove everything bundled — the virgin app should only use what's actually made on the phone"), needs a Digest+Plan pass before it's executable (only a short stub brief exists) | see `QUESTIONS-FOR-NATHAN.md` |
| G | Specifications / route variants on an existing Way | Not started — **Q2 answered** (rides auto-group by From→To→Spec1→Spec2… prefix, splitting only where names actually diverge), needs a Digest+Plan pass before it's executable (only a short stub brief exists) | see `QUESTIONS-FOR-NATHAN.md` |
| H | Ride detail screen (RIDES tap + post-stop destination) | Not started — **C's map-side scope landed 2026-09-02** (unblocked), **Q4 answered** (drop the RESULTS tab entirely), still needs a Digest+Plan pass (only a short stub brief exists) plus a nav-model call | see `QUESTIONS-FOR-NATHAN.md` |
| I | Gate card on the map + finger scrub | Not started — **C's map-side scope landed 2026-09-02** (unblocked), **Q1 answered** (keep the +/- pad, but it must move the gates on the real ride line, not today's straight render — an added bug to fix), needs a Digest+Plan pass (only a short stub brief exists) | see `QUESTIONS-FOR-NATHAN.md` |
| K | Sector-coloured trail, phase 2 (live map) | Not started — **C landed 2026-09-02** (unblocked), **Q7 answered** (yes — apply in the live racing map and the RIDES-tab ride popup too, plus consider an all-yellow-vs-sectors options toggle), needs a Digest+Plan pass (only a short stub brief exists) | see `QUESTIONS-FOR-NATHAN.md` |
| M | RECORD setup layout (tight-and-grows vs fixed) | Not started — **Q5 answered** (tight-and-grows, plus the live map back — already satisfied by WP-D), needs a Digest+Plan pass to confirm nothing else is left (only a short stub brief exists) | see `QUESTIONS-FOR-NATHAN.md` |
