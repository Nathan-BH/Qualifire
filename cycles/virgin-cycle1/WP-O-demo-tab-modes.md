**Status: DONE — both phases landed on the device 2026-09-02.** `src/ui/demoModel.ts` (new, pure) plus `DemoScreen.tsx`'s two-pill FIRST RIDE / SECOND RIDE picker landed in one pass, since WP-D and WP-J were already in by the time this was executed. `sectorColours` replaces `gateColours` entirely in SECOND RIDE; FIRST RIDE uses `DEMO_FIRST_RIDE_ID` (non-null, no manifest entry) + the WP-J `trail` prop, no buzz, no `LiveSectorPane`. New `tests/demo_suite.ts` (7 tests, all passing) pins the fixture's tier verdicts and the gate-progression rule for `demoSectorColours`. Verification ran in the cloud container (`device_bash` down this session): 333 tests, 330 pass, 0 fail, 3 skip (326/323/0/3 baseline + 7 new). One test-harness-only wrinkle: `demoModel.ts` statically imports `colourModel.ts`, which statically imports the app's bare-JSON seed — the same Node ESM import-attribute wall `live_colour_suite.ts` already solved — so `demo_suite.ts` uses that same `registerHooks` + dynamic-import pattern; no production code changed to work around it. `tsc --noEmit` fails only on the pre-existing `expo/tsconfig.base` resolution gap (no `node_modules` here), same as every other suite — nothing it reports names `DemoScreen.tsx` or `demoModel.ts`. On-device visual check (§4's two on-device check lists) still outstanding — no device shell this session.
