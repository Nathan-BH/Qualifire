**Status: Brief written 2026-09-03 (Digest+Plan pass). Ready to execute — Nathan's Q6 answer ("remove everything bundled") is fully incorporated; no decision from Nathan needed (one pre-approved judgment call — DEMO keeps replaying the Morning geometry via its own frozen fixture — is recorded in §7.1 for the record, not for a ruling). A single build-mode guard in `store/seed.ts` (`bundledForSeedMode`) empties the bundled route manifest AND the three bundled route PNGs at their only two definition sites in `routeMapView.tsx`, so on a `SEED_MODE === 'empty'` build no code path can resolve, draw or reference a bundled route; the DEMO tab stops importing `routes.json` and gets its own small decoupled fixture (`src/ui/demoRouteFixture.ts`) handed to the map through a new `asset` prop.**
