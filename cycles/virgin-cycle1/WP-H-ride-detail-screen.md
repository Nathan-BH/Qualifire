**Status: DONE — landed on the device 2026-09-04, independently inspected (fresh Fable pass, clean; one test-coverage addition). 438 tests, 435 pass, 0 fail, 3 skip (29 new); `tsc --noEmit` exit 0. RESULT tab retired (moved to `safe_to_delete/`, never deleted); new `RideDetailScreen.tsx` is both the post-STOP landing and what a RIDES-row tap opens; `promoteRideToReference` (`wayFromRide.ts`) implements Nathan's reset-not-remap answer to §8.1 behind a one-step warning dialog; the create-way button (built by a follow-up Fable ruling after Execute correctly stopped on a real WP-G conflict) now follows `draft.existingWayId` so it offers a new way or a variant on an existing one, whichever the ride actually matches. `RecordScreen.tsx` thinned to a caller of the shared `wayFromRide.ts` module (also closes the WP-F §8 duplication debt).**
