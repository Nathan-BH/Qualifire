**Status: DONE. Landed on the device (`app/src/live/engine.ts`, `app/tests/live_suite.ts`) on 2026-09-05, commit `2fe0ede`. `feedCandidate` now captures `wasOnRoute` (the previous fix's verdict) before `c.proj.update()` rewrites it, and discounts the jump whenever `jump > REACQ_JUMP_M || !wasOnRoute` — closing the sub-245m residual by cause instead of size. `core/src/live.ts` untouched, as required. Verified together with WP-E/WP-F/WP-G: 472 tests, 469 pass, 0 fail, 3 skip; `tsc --noEmit` exit 0.**
