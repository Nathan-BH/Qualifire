**Status: DONE. Landed on the device (`app/src/store/wayFromRide.ts`, `app/src/ui/RoutesScreen.tsx`, `app/src/ui/gateAdjustCard.tsx`, `app/tests/waycreation_suite.ts`) on 2026-09-05, commit `0b45803`. New `editRouteGates()`/`gateEditDraftFor()` reuse `promoteRideToReference`'s reset-not-remap convention (Nathan's ratified Q2 design) but keep the reference line untouched. Also fixed a nested-Pressable footgun on the way card (any touch inside the open body used to collapse it). Verified together with WP-M: 495 tests, 492 pass, 0 fail, 3 skip; `tsc --noEmit` exit 0. **Superseded in place, same session:** WP-J's extended scope (Nathan's follow-up instruction) moved the actual edit-gates UI out of `RoutesScreen.tsx`'s inline accordion into a new full-screen `GateAdjustScreen.tsx` (commit `4524122`); WP-K then slimmed `RoutesScreen.tsx` to a tap-only list and moved the entry point onto `CatalogDetailScreen.tsx` (commit `1a3e99c`). This WP's *data-layer* contribution (`editRouteGates()`/`gateEditDraftFor()`) is still exactly what both later WPs call — only the UI acceptance criteria's location changed. UI acceptance criteria still want Nathan's on-device look, now on the way-detail screen rather than an expanded accordion card.**
