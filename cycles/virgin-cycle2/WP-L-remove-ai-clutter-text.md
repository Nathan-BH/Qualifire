**Status: DONE. Landed on the device (`app/src/ui/DemoScreen.tsx`, `RecordScreen.tsx`, `RideDetailScreen.tsx`, `RoutesScreen.tsx`, `settings.tsx`) on 2026-09-05, commit `f3b47cd`. Part A: all 8 clutter strings removed/trimmed (Nathan's 4 named examples + 4 sweep-found ones), doc-reference fragments (`§18, unsettled`/`D-019`/`§21`) scrubbed from user-visible hint copy. Part B: SETTINGS rows now hide their hint behind a small circled `?`, tap to reveal, one row open at a time — applied to all 11 rows including "Timing" (added by WP-C after this brief was written; its hint copy untouched, flagged for Nathan). Verified: 506 tests, 503 pass, 0 fail, 3 skip (unchanged — no suite reads UI copy); `tsc --noEmit` exit 0. The `?` tap target/legibility/toggle feel needs Nathan's on-device look.**
...
| 2 | `app/src/ui/RoutesScreen.tsx:238–241` | `Route lines are pre-rendered from your own rides, with the measured gates marked. Editing a route's gates resets its history — every past ride is re-timed against the new gates.` | inline `{ color: t.textDim, fontSize: 11.5, marginTop: 4 }`, last child of the ScrollView |
...
2. `RoutesScreen.tsx` — remove the `Route lines are pre-rendered…` `<Text>` (last child of the ScrollView, immediately after the WAYS `.map()`). Nothing else changes; `paddingBottom: 40` on the ScrollView already gives the last card breathing room. **Re-anchored 2026-09-05:** this string's wording changed under WP-J (now "Editing a route's gates resets its history — every past ride is re-timed against the new gates." — see updated A1 table); still the last child of the ScrollView, still deleted whole, no other change.
...
## Update 2026-09-05 (coordinator re-anchor, pre-execute)

Re-verified all A1/A2/A3 anchor strings against the repo as it stands after WP-A/B/I/J(extended)/K/M/C have all landed. `RoutesScreen.tsx` was rewritten 74% by WP-K (tap-only list, no more inline accordion/edit-gates/delete code), but the two captions this brief targets (YOUR PLACES dormant-places note; the ScrollView-trailing route-lines note) both survived that rewrite unchanged in structural position (last child of their respective containers) — WP-K only removed the per-row expand/detail code, not these two screen-level captions.

One wording drift found and fixed above: anchor #2's quoted string used the pre-WP-J sentence ("Moving a middle gate keeps lap history comparable; moving START or FINISH does not.") — WP-J's extended scope (real-map gate editor, WP-J update Part B) changed gate-edit semantics so *any* gate edit now resets history, and the shipped copy already reflects that ("Editing a route's gates resets its history — every past ride is re-timed against the new gates."). Brief corrected to match; the fix is still a whole-element deletion, nothing else about item 2 changes.

All other anchors (#3 RideDetailScreen.tsx `Position is a fact…`, #4/#5 DemoScreen.tsx, #6 RideDetailScreen.tsx `Free-ride sector times…`, #7 settings.tsx `Saved on the phone…`, #8 RecordScreen.tsx `(§8a)`, #9 settings.tsx doc-ref hints) were grepped verbatim against the current repo and all match the brief exactly, same file, same neighboring code shape. Execute may proceed on the brief as originally written plus the item-2 correction above.
