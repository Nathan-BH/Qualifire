**Status: DONE. Landed on the device (`app/src/ui/wayNamingCard.tsx`, `app/tests/waycreation_suite.ts`) on 2026-09-05, commit `eaab0a4`. G1: the loop sub-copy now names `props.startExistingLabel` when the loop point is an existing landmark, falling back to "one new place" otherwise. G2: two regression tests added ("WP-G 9"/"WP-G 10"), re-derived against the real 882-line test file after Inspect found the original spec targeted an unreachable/duplicate branch. Verified together with WP-E/WP-F/WP-D: 472 tests, 469 pass, 0 fail, 3 skip; `tsc --noEmit` exit 0.**
