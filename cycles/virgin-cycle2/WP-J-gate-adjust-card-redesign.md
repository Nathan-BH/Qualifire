**Status: DONE. Landed on the device (`app/src/ui/gateAdjustCard.tsx`, `gateAdjustModel.ts`, `routeMapView.tsx`, `tabNav.tsx`, `App.tsx`, new `GateAdjustScreen.tsx`, `RoutesScreen.tsx`, `RecordScreen.tsx`, `RideDetailScreen.tsx`) on 2026-09-05, commit `4524122`. Extended scope (this session's coordinator, per Nathan's own follow-up instruction): the card now embeds a real `RouteMapView` with a 5-chip gate selector and hold-to-repeat nudge pad, replacing the custom-drawn line; a NEW full-screen `GateAdjustScreen.tsx` (opened via `tabNav.openGateAdjust({routeId})`, the same "screen owns intent, Shell owns chrome" mount-swap pattern as `RideDetailScreen.tsx`) now owns the edit-gates flow that WP-I had put inline in `RoutesScreen.tsx`'s accordion — pressing "edit" from ROUTES opens this dedicated full-screen map editor instead. `gateAdjustMapModel.ts`/`gateadjustmap_suite.ts` retired to `safe_to_delete/` (content-identical, never deleted). Verified: 484 tests, 481 pass, 0 fail, 3 skip; `tsc --noEmit` exit 0. Map/gesture feel (chip selection, hold-to-repeat pacing) still needs Nathan's on-device look — superseded further by WP-K, which relocated the entry point onto `CatalogDetailScreen.tsx`.**
