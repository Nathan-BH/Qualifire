**Status: DONE. Landed on the device (`app/src/ui/chips.tsx`, `app/src/ui/rideDetailModel.ts`, new `app/src/ui/tierColour.ts`) on 2026-09-05, commit `644cb04`. `tierLineColour`/`YELLOW_TIER` extracted verbatim into the new pure `.ts` module; `chips.tsx` re-exports both so every existing `.tsx` consumer is unaffected; `rideDetailModel.ts`'s hand-duplicated `lineColourFor` is gone. The brief's one open question (Tier/UiTier compatibility) was resolved by Inspect before execution — no cast needed. Verified together with WP-E/WP-G/WP-D: 472 tests, 469 pass, 0 fail, 3 skip; `tsc --noEmit` exit 0.**
