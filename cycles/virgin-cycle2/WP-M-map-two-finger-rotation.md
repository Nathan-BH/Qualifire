**Status: DONE. Landed on the device (`app/src/ui/routeMapGeo.ts`, `app/src/ui/routeMapView.tsx`, `app/tests/routemap_suite.ts`, `app/tests/routemapgeo_suite.ts`) on 2026-09-05, commit `6c3d6ab`. New `rotateEnabledFor()` gates `touchRotate` on for browse/prestart/finished; a held `userBearing`, read back from `onRegionDidChange`, composes into `cameraTargetFor()` so +/-/FIT/ME never silently discard rotation; a new compass button resets to north via an imperative camera-ref call. The Inspect-tier correction (passing `rotateEnabled ? userBearing : null` directly into the same render's camera push, since RecordScreen reuses the same map instance across prestart-to-moving) was applied as specified. PNG map rung untouched. Verified together with WP-I: 495 tests, 492 pass, 0 fail, 3 skip; `tsc --noEmit` exit 0. Needs Nathan's on-device feel-check (gesture responsiveness, compass glyph).**
