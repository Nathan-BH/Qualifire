**Status: DONE. Landed on the device (new `app/src/store/timing.ts`, `app/src/store/types.ts`, 9 call-site files, `app/src/ui/settings.tsx` new Timing row) on 2026-09-05, commit `ed0a5fe`. `TowerRow.movingS` renamed to `timeS`; 27 real call sites rewired to the new `scoredS()` helper (raw time is the default, moving time an opt-in via Settings). `setTimingMode(s.timing)` is called synchronously in the settings provider's render body (not an effect) so same-render consumers never lag one frame behind — commented in place. Verified: 494 tests, 491 pass, 0 fail, 3 skip; `tsc --noEmit` exit 0.**
