**Status: DONE. Landed on the device (`app/src/storage/jsonl.ts`, `app/src/store/wayFromRide.ts`, `app/src/live/userRefs.ts`, `app/src/store/resultsStore.ts`, `app/src/storage/gpxExport.ts`, `app/src/storage/gpxPlusExport.ts`) on 2026-09-05, commit `495b3f8`. Parts A1-A5 landed as specified; Part C (location task handler) deliberately deferred — the brief itself frames it as separable prevention, not part of the reported-bug fix. A fresh Fable ruling resolved one test-fixture ambiguity the Execute agent correctly stopped on (a pre-existing 'stationary knot' test fixture had physically-impossible overlapping timestamps and only passed by relying on the exact bug this WP fixes — the fixture was corrected, not the new chronological-sort code). Verified together with WP-A: 483 tests, 480 pass, 0 fail, 3 skip; `tsc --noEmit` exit 0. On-device GPX+ verification against the WorkHomeWet/281e ride still needs Nathan's own look.**
