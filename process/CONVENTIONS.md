...
## Nathan-facing docs (added 2026-08-24, cycle 024)

- **`NATHAN-STATUS.md` is regenerated by the Principal at every cycle end**, same pass as `STATE.md` — a cycle that ends without it is unfinished.
- **Design round-trip check, at cycle start:** diff `design/edited/` against `design/canonical/` (see `design/README.md` for the full workflow WP-J set up) and turn any real differences into agenda items before the cycle's other work begins. Nathan's edits in `edited/` are never overwritten by this check — it only reads.
- **`HOW-THE-APP-IS-BUILT.md` and `GLOSSARY.md`** (both written 2026-08-24, cycle 024) are updated whenever structure or vocabulary changes, on the same standing as this section.
