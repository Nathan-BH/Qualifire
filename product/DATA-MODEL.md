...
*Historical. Q1 and Q4 are about Nathan's archive landmarks (not on this branch). Q2: loops carry a hand label (`loopDiscriminator`). Q3 ("does a way with 4 rides get colours?") was ruled YES by D-045 (2026-08-26) — colours from ride 1 — and this is now built (NW-1, 2026-09-08: `MIN_HISTORY = 1`), with the one refinement the ruling itself made explicit: ride 1 (the way's reference ride) gets a rank but no colour on the day it's ridden, since it never raced anyone yet — like any stored ride it can pick up a colour later once the window it's judged against changes.*
