...
| B-31 cold-start ladder — verdict-free ride-1 board, "ride n of 5", two announcements | **CORE MECHANIC BUILT** (NW-1, 2026-09-08): D-045 ruling 1's floor is in code now — `MIN_HISTORY = 1` (`colourModel.ts:47`) — so the way's reference ride (ride 1) is verdict-free (neutral) on the day it's ridden while still ranking once stored; the next ride is purple/yellow-only, and green opens up from the ride after that. Like any stored ride, the reference ride re-enters the judged pool once later rides exist (window exclusion is by id, not by time), so it can pick up a colour on a later visit — "verdict-free" describes the moment it's ridden, not a permanent exemption. The ladder's specific UI framing ("ride n of 5" copy, two scripted announcements) was never part of the ruling and is not built — F-4's messaging half stays open. | `app/src/ui/colourModel.ts`, `app/tests/live_colour_suite.ts` |
...
**BUILT as of NW-1 (2026-09-08), correcting this note's own earlier wording:** MIN_HISTORY is 1, not 5 (D-045 ruling 1, 2026-08-26, executed 2026-09-08). The corrected shape: the way's reference ride (ride 1) is verdict-free (neutral) on the day it's ridden but earns a rank; one prior ride ⇒ purple/yellow only; 2+ prior rides ⇒ full model on the average. This note previously (and wrongly) said "the first ride ever on a route logs all-purple sectors" — it does not; that described the old Strava-seeded world, not a genuinely blank install. N=10 and the palette are unaffected. Later references to MIN_HISTORY elsewhere in this file (the §-tables and F-4's empty-state premise) are stale for the same reason — read `colourModel.ts` for the current behaviour rather than this file's tables.
