...
| B-46 | **MapLibre install spike, costing zero EAS builds** — branch, `expo install`, plugin entry, local `expo prebuild --clean --platform android`, `tsc --noEmit`, `expo export --platform android`, read the generated manifest. Settles the only two things reading cannot: resolution under this repo's Metro conventions, and whether the native build links. Condition 1 of D-035/D-041. **Ungated by D-041** — runs now, as `scripts/spike-maplibre.ps1` on Nathan's PC (the cloud sandbox cannot reach npm) | Mobile Dev + Nathan | NEEDS NATHAN — run the script |
...
| B-50 | **MapLibre + OpenFreeMap vector base replacing `routeMapView.tsx`'s PNG compositor**; route/gates/dot re-implemented as GeoJSON layers per `product/MAP-TILES.md` §5 (v11 declarative API); PNG kept as the failure fallback rung; needs one dev-client rebuild after B-46 passes | Mobile Dev | OPEN — blocked on B-46 |
