...
| B-50 | **MapLibre + OpenFreeMap vector base replacing `routeMapView.tsx`'s PNG compositor**; route/gates/dot re-implemented as GeoJSON layers per `product/MAP-TILES.md` §5 (v11 declarative API); PNG kept as the failure fallback rung; needs one dev-client rebuild after B-46 passes | Mobile Dev | OPEN — B-46 PASSED, ready to brief |
| B-51 | **Wire the real map onto every screen per `product/MAP-CONTRACT.md`**: Record's four ride states (pre-start / moving / stopped-at-light / finished), Demo (shared component), Result's new view-trace link (B-57), Routes overview's static-Image swap | Mobile Dev + Designer | OPEN — blocked on B-50 |
...
| B-53 | **"Map data sources" attribution sheet** (Strava-style), replacing the in-corner credit string; owed strings in `product/MAP-TILES.md` §6 | Designer + Mobile Dev | OPEN |
...
| B-57 | **Result screen "view trace" link** (LAYOUT §3 rule 6) — unbuilt today; worth building now that it opens onto a real map | Designer + Mobile Dev | OPEN |
| B-58 | **Phone-checkable acceptance test for "the map problem is solved"** (`product/MAP-CONTRACT.md` §4), folded into `app/README-dev.md`'s acceptance steps | QA + Product Owner | OPEN |
