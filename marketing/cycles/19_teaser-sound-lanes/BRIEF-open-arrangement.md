...
| HOWTO array | 509–517 | insert ONE line as the new 8th entry (after the "Copy list gives…" line, before the "Lanes A … and B" line) — §4.5 |
...
- Drop handler (1251): BEFORE `fromDrop`, `const fl = e.dataTransfer.files; if (fl && fl.length === 1 && /\.(json|txt)$/i.test(fl[0].name)) { if (!S.loaded) { say("open the kit folder first, then the arrangement", "err"); return; } await openArrangementFile(fl[0]); return; }`. Everything else stays the kit path.
...
- HOWTO new 8th line (exact): `"Save arrangement downloads your clips and mutes as a small .json file; keep it in teaser/arrangements/. Open arrangement (or dropping the file on the page) brings a saved .json, or a pasted clip list saved as .txt, back onto the lanes; Undo in the status line brings the previous clips back."` The how-to has the "press ? for the how-to" fallback when it does not fit (page 1054–1057), so a 9th line is safe; the `#help-panel` list shows all nine.
- Keys: none added.
...
12. Hygiene additions: `html` contains `id="btn-open-arr"`, `id="btn-save-arr"`, `id="btn-undo"`, `id="arr-input"` exactly once each; `HOWTO` array has 9 entries: take the text between `const HOWTO = [` and the next `];`, count its lines that start with four spaces and a `"` → 9.
...
| HOWTO ≤ 8 lines (cycle brief §6.7) | Now 9; the overflow fallback exists. If `howtoShown` flips to false at 1440x900 in `e2e.mjs`, report it (not a STOP): the `?` panel has the full text. |
...
- The how-to gets one more line (nine); the `?` panel shows all of them.
