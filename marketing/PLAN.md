...
**Status: a plan, mostly built, none of it freshly rendered.** Restructured
2026-09-08 (virgin-cycle5, `BRIEF-website-improvement-plan.md` §(d)) per
Nathan's own ruling that HyperFrames is *not* a priority — but this cycle
(2026-09-09) went ahead and built four compositions anyway: teaser, gate,
purple and tour. The teaser got one render early in the day (2026-09-09
09:14), before its later fade/hard-kill fix; the other three, and the fixed
teaser, have not been rendered at all — `device_bash` (the remote shell to
Nathan's PC) was unavailable this whole session, so no `render.ps1` runs
happened here. Nathan needs to render all four himself: `render.ps1 -Name
<comp> -Render` per COMMANDS.md, for `<comp>` in teaser/gate/purple/tour.
What this file is for: when someone picks the slate up later, it
...
**2026-09-09 update:** ideas 1, 3 and 4 are now built (not yet rendered),
idea 2 was revised per Nathan's feedback and rendered once before that
revision (needs a fresh render), and idea 7 was retired and folded into
idea 3 — see the per-idea status notes below and
`cycles/01_first-slate/IDEAS-AND-FEEDBACK.md` for the full feedback record.
...
| # | Status | Working title | Length | Workflow skill | Needs | Blocked on |
|---|---|---|---|---|---|---|
| 1 | BUILT | The Gate (logo sting) | ~4–6s | `/motion-graphics` | logo SVG only | nothing — built 2026-09-09, not yet rendered |
| 2 | BUILT | Reframed (brand teaser) | ~12s | `/motion-graphics` | already built | nothing — rendered once 2026-09-09, revised since per Nathan's feedback, needs a fresh render |
| 3 | BUILT | Qualifire, a tour | ~45–60s | `/product-launch-video` | `marketing/index.html` | nothing — built 2026-09-09, not yet rendered, restructured per feedback |
| 4 | BUILT | Why purple is rare | ~40–60s | `/faceless-explainer` | GLOSSARY.md + `colourModel.ts` | nothing — built 2026-09-09, not yet rendered, awaiting Nathan's feedback |
| 5 | open | First ride, first route | ~30–45s | `/general-video` | on-device screen footage of a blank install | that footage does not exist yet (OPEN-ITEMS.md items 1–3) |
| 7 | RETIRED — merged into 3 | P2 of your last 10 | ~20–30s | `/faceless-explainer` or `/general-video` | `tower.tsx` choreography + on-device footage | retired 2026-09-09, folded into idea 3's beats 4-5 |
...
**Status: BUILT.** Updated 2026-09-09 per Nathan's feedback: mark now fades
out in place (no shrink-to-corner) and fades back in for the endcard (no
travel); 3 `gsap_exit_missing_hard_kill` contract errors fixed via `.inner`
wrapper + `tl.set` hard kills (see StaticGuard errors he saw in the preview
...
09:14); the fixed version below still needs its own.

*Blocked on.* Nothing except someone choosing to spend the render — a fresh
one, since the fade/hard-kill fix postdates the one render this idea has.

### 3 · Qualifire, a tour

**Status: BUILT.** Substantially restructured 2026-09-09 per Nathan's direct
feedback — the original site-scroll-cut-sequence approach was dropped; now a
...
- **15-22s** — Landmarks (start/end) and three automatic sector gates pop in.
- **22-35s** — Second ride: the dot travels again, gates pulse as it crosses
  them; a rank fragment ("P1 of 2", purple) slots in over the dimming map.
- **35-45s** — The full timing tower fills to 10 rows.
- **45-50s** — Close: mark, "QUALIFIRE", "Same road. New meaning."

**Superseded 2026-09-09 — kept for history, see the 2026-09-09 status note
...
**Cross-ref (2026-09-09):** idea 3's beats 2-3 now cover this narrative
abstractly as a design mock (no real footage); idea 5 remains the option to
swap in real footage of the same story later. No rush, not retired.

### 7 · P2 of your last 10

**Status: RETIRED.** Retired 2026-09-09, folded into idea 3's beat 4
(ranking reveal) and beat 5 (tower fill) per Nathan's direct feedback — no
...
1. **Reframed (2), then The Gate (1).** Both built 2026-09-09. Reframed was
   rendered once that day, before its later fade/hard-kill fix, so it needs
   a fresh render; The Gate has not been rendered at all.
2. **Qualifire, a tour (3).** Built 2026-09-09, not yet rendered —
   restructured into a 6-beat abstract narrative per Nathan's direct feedback.
3. **Why purple is rare (4).** Built 2026-09-09, not yet rendered, awaiting
   Nathan's feedback.
4. ~~P2 of your last 10 (7).~~ Retired 2026-09-09, merged into idea 3's
   beats 4-5.
5. **First ride, first route (5).** Last — it is *only* footage, and the
   footage waits on OPEN-ITEMS.md items 1–3. Unchanged this cycle.

Idea 5 gets better after Nathan's on-device passes, which are the real gate
on this remaining slate item. Nothing in this list has a date.
