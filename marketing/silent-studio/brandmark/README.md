...
`silent-studio/gates-saving/`) — that name collision between "gate"
(this mark) and "gates" (sector markers) was part of why this got renamed.

## Renamed 2026-09-10

This composition was `silent-studio/gate/`. It is kept there as a legacy duplicate
only because this session has no way to delete files from Nathan's PC —
`silent-studio/brandmark/` is canonical going forward; treat `silent-studio/gate/` as
superseded.
...
On your PC, from `marketing/silent-studio/`:
```powershell
...
- **Teaser's opening (`silent-studio/brandmark/opening/`):** mark draws (ring +
...
- **Teaser's closing (`silent-studio/brandmark/closing/`):** the mark fades
