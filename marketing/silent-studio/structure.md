...
with-sound files got flagged at first, and `silent-studio/all-renders/`'s superseded
...
1. copy the new render into `silent-studio/all-renders/<comp>_vN.mp4` (replacing the
...
- remove stray test files `silent-studio/ZZTEST_render.mp4` and `brandmark/renders/TESTFILE_temp.mp4` (same: move, don't delete)
