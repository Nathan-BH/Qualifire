...
2. **Every task runs the model-tier protocol (D-039).** `process/CONVENTIONS.md` → "Model tiers": Haiku subagent triages; the frontier model reads the code itself and writes a self-contained brief; a Sonnet subagent executes from the brief alone under a stop-on-ambiguity rule; a fresh-context frontier subagent inspects adversarially and reruns every check. If this chat runs on Sonnet (coordinator mode), planning and inspection are dispatched as `model: "fable"` subagents, and executor escalations are forwarded verbatim to a Fable subagent — the Sonnet coordinator never rules on ambiguity itself. Chores under ~10 mechanical lines skip the tiers — subagent overhead (~30–80k tokens) must pay for itself. **Narrate the tiers in chat**: announce each dispatch (tier, model, mandate), surface escalations, end with a tier/model/tokens/outcome readout table.
