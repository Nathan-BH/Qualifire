...
- Wrote `product/MAP-CONTRACT.md` §1–4: per-surface contract (RouteMapView called only from RecordScreen/DemoScreen; RoutesScreen static Image; ResultScreen no map today), Strava feature verdicts (streets ADOPT, satellite LATER, 3D NON-GOAL, layer switcher LATER, data-sources sheet ADOPT).
- Proposed B-50…B-58 (accepted into BACKLOG by the Principal), flagged B-34/B-46/B-47/B-32 changes, wrote the phone acceptance test.
