...
- Wrote `product/MAP-STACK-OPTIONS.md` comparing MapLibre RN, expo-maps, @rnmapbox/maps, WebView+JS map, status-quo PNG for map-on-every-screen-incl-live per Nathan's rulings; ranked MapLibre #1 (no account/key, one rebuild, PMTiles/ambient cache), expo-maps backup gated on the billing-card question.
- Wrote `scripts/spike-maplibre.ps1` (not run): zero-EAS-build local prebuild/typecheck/export spike on branch `spike/maplibre`. Battery at 1 Hz on the live screen stays [ESTIMATE] — B-47 A/B is more urgent now the live screen is in scope.
- Inspector corrected v10→v11 Camera prop names and the 3D-terrain cell.
